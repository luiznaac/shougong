import { useCallback, useEffect, useMemo, useRef, useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { useQuery } from "@tanstack/react-query";
import { api, ApiError } from "../api/client.ts";
import type { SrsRating, StudyItem } from "../api/types.ts";
import { Pinyin } from "../components/Pinyin.tsx";

type Phase = "prompt" | "revealed";

const RATINGS: { key: SrsRating; label: string; hotkey: string; className: string }[] = [
  { key: "again", label: "Errei", hotkey: "1", className: "bg-rose-600 hover:bg-rose-500" },
  { key: "hard", label: "Difícil", hotkey: "2", className: "bg-amber-600 hover:bg-amber-500" },
  { key: "good", label: "Bom", hotkey: "3", className: "bg-emerald-600 hover:bg-emerald-500" },
  { key: "easy", label: "Fácil", hotkey: "4", className: "bg-sky-600 hover:bg-sky-500" },
];

export function Review() {
  const navigate = useNavigate();

  // Snapshot the due queue once, on entry.
  const { data, isLoading, error } = useQuery({
    queryKey: ["review-queue"],
    queryFn: () => api.listAllStudyItems({ due: true }),
    staleTime: Infinity,
    gcTime: 0,
  });

  const [queue, setQueue] = useState<StudyItem[] | null>(null);
  const [idx, setIdx] = useState(0);
  const [phase, setPhase] = useState<Phase>("prompt");
  const [done, setDone] = useState(0);
  const [correct, setCorrect] = useState(0);
  const [skipped, setSkipped] = useState(0);
  const [submitting, setSubmitting] = useState(false);
  const [toast, setToast] = useState<string | null>(null);

  useEffect(() => {
    if (data && queue === null) setQueue(data);
  }, [data, queue]);

  const current = queue?.[idx];
  const remaining = queue ? queue.length - idx : 0;
  const accuracy = done > 0 ? Math.round((correct / done) * 100) : 100;

  const advance = useCallback(() => {
    setPhase("prompt");
    setIdx((i) => i + 1);
  }, []);

  const grade = useCallback(
    async (rating: SrsRating) => {
      if (!current || submitting) return;
      setSubmitting(true);
      try {
        await api.reviewStudyItem(current.id, rating);
        setDone((n) => n + 1);
        if (rating !== "again") setCorrect((n) => n + 1);
        advance();
      } catch (e) {
        if (e instanceof ApiError && e.status === 409) {
          setToast("Item não estava mais due — pulado.");
          setSkipped((n) => n + 1);
          advance();
        } else {
          setToast(`Erro ao enviar: ${e instanceof Error ? e.message : String(e)}`);
        }
      } finally {
        setSubmitting(false);
      }
    },
    [current, submitting, advance],
  );

  const reveal = useCallback(() => setPhase("revealed"), []);

  // Keyboard shortcuts.
  const phaseRef = useRef(phase);
  phaseRef.current = phase;
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.repeat) return;
      if (e.key === "Escape") {
        navigate("/");
        return;
      }
      if (phaseRef.current === "prompt") {
        if (e.key === " " || e.key === "Enter") {
          e.preventDefault();
          reveal();
        }
      } else {
        const hit = RATINGS.find((r) => r.hotkey === e.key);
        if (hit) {
          e.preventDefault();
          void grade(hit.key);
        }
      }
    };
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [reveal, grade, navigate]);

  useEffect(() => {
    if (!toast) return;
    const t = setTimeout(() => setToast(null), 2500);
    return () => clearTimeout(t);
  }, [toast]);

  const finished = queue !== null && idx >= queue.length;

  const body = useMemo(() => {
    if (isLoading || queue === null)
      return <p className="text-slate-400">Carregando fila…</p>;
    if (error)
      return <p className="text-rose-400">Falha ao carregar: {String(error)}</p>;
    if (queue.length === 0)
      return <Empty message="Nenhum item para revisar agora." />;
    if (finished)
      return (
        <Summary done={done} correct={correct} skipped={skipped} accuracy={accuracy} />
      );
    if (!current) return null;

    return (
      <div className="w-full max-w-xl">
        {phase === "prompt" ? (
          <PromptCard item={current} onReveal={reveal} />
        ) : (
          <RevealCard item={current} onGrade={grade} disabled={submitting} />
        )}
      </div>
    );
  }, [
    isLoading, queue, error, finished, current, phase, done, correct, skipped,
    accuracy, reveal, grade, submitting,
  ]);

  return (
    <div className="flex min-h-screen flex-col bg-slate-950">
      <TopBar
        remaining={remaining}
        done={done}
        accuracy={accuracy}
        onExit={() => navigate("/")}
      />
      <div className="flex flex-1 flex-col items-center justify-center px-4 py-10">
        {body}
      </div>
      {toast && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 rounded-md bg-slate-800 px-4 py-2 text-sm text-slate-100 shadow-lg">
          {toast}
        </div>
      )}
    </div>
  );
}

function TopBar({
  remaining,
  done,
  accuracy,
  onExit,
}: {
  remaining: number;
  done: number;
  accuracy: number;
  onExit: () => void;
}) {
  return (
    <div className="flex items-center gap-4 border-b border-white/10 bg-slate-900/60 px-4 py-2 text-sm">
      <button onClick={onExit} className="text-slate-400 hover:text-slate-200" title="Sair (Esc)">
        ← Sair
      </button>
      <div className="ml-auto flex items-center gap-4 tabular-nums text-slate-300">
        <span title="Concluídos">✓ {done}</span>
        <span title="Restantes">☰ {remaining}</span>
        <span title="Acertos">👍 {accuracy}%</span>
      </div>
    </div>
  );
}

function PromptCard({ item, onReveal }: { item: StudyItem; onReveal: () => void }) {
  return (
    <div className="flex flex-col items-center gap-8 text-center">
      <p className="text-sm uppercase tracking-widest text-slate-500">
        Escreva o caractere à mão
      </p>

      <Pinyin value={item.entry.pinyin} className="text-5xl font-light sm:text-6xl" />

      <ul className="space-y-1 text-lg text-slate-300">
        {item.entry.definitions.slice(0, 5).map((d, i) => (
          <li key={i}>{d}</li>
        ))}
      </ul>

      <div
        className="flex h-40 w-40 items-center justify-center rounded-xl border-2 border-dashed border-white/15 font-hanzi text-7xl text-slate-700 select-none"
        aria-hidden
      >
        ?
      </div>

      <button
        onClick={onReveal}
        className="rounded-lg bg-slate-100 px-8 py-3 font-semibold text-slate-900 hover:bg-white"
      >
        Revelar <kbd className="ml-2 rounded bg-slate-300 px-1.5 text-xs">espaço</kbd>
      </button>
    </div>
  );
}

function RevealCard({
  item,
  onGrade,
  disabled,
}: {
  item: StudyItem;
  onGrade: (r: SrsRating) => void;
  disabled: boolean;
}) {
  return (
    <div className="flex flex-col items-center gap-6 text-center">
      <span lang="zh-Hans" className="font-hanzi text-[10rem] leading-none text-slate-50">
        {item.entry.simplified}
      </span>
      <Pinyin value={item.entry.pinyin} className="text-3xl font-light" />
      <ul className="space-y-0.5 text-slate-400">
        {item.entry.definitions.slice(0, 5).map((d, i) => (
          <li key={i}>{d}</li>
        ))}
      </ul>

      <p className="pt-2 text-sm text-slate-500">Você acertou a escrita?</p>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {RATINGS.map((r) => (
          <button
            key={r.key}
            disabled={disabled}
            onClick={() => onGrade(r.key)}
            className={`rounded-lg px-5 py-3 font-semibold text-white transition-colors disabled:opacity-50 ${r.className}`}
          >
            {r.label}
            <kbd className="ml-2 rounded bg-black/20 px-1.5 text-xs">{r.hotkey}</kbd>
          </button>
        ))}
      </div>
      <Link to={`/items/${item.id}`} className="text-xs text-slate-500 hover:text-slate-300">
        ver página do item
      </Link>
    </div>
  );
}

function Empty({ message }: { message: string }) {
  return (
    <div className="flex flex-col items-center gap-4 text-center">
      <p className="text-lg text-slate-300">{message}</p>
      <Link to="/" className="text-sm text-accent-500 hover:underline">
        Voltar ao painel
      </Link>
    </div>
  );
}

function Summary({
  done,
  correct,
  skipped,
  accuracy,
}: {
  done: number;
  correct: number;
  skipped: number;
  accuracy: number;
}) {
  return (
    <div className="flex flex-col items-center gap-6 text-center">
      <h1 className="text-2xl font-bold text-slate-100">Sessão concluída 🎉</h1>
      <div className="flex gap-8 text-slate-300">
        <Metric label="Revisados" value={done} />
        <Metric label="Acertos" value={correct} />
        <Metric label="Precisão" value={`${accuracy}%`} />
        {skipped > 0 && <Metric label="Pulados" value={skipped} />}
      </div>
      <Link
        to="/"
        className="rounded-lg bg-accent-500 px-6 py-2.5 font-semibold text-white hover:bg-accent-600"
      >
        Voltar ao painel
      </Link>
    </div>
  );
}

function Metric({ label, value }: { label: string; value: number | string }) {
  return (
    <div>
      <div className="text-3xl font-bold tabular-nums text-slate-100">{value}</div>
      <div className="text-xs uppercase tracking-wide text-slate-500">{label}</div>
    </div>
  );
}
